...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordBpfTCPRxDataPoint(ts, 1, 3, "comm-val", "saddr-val", "daddr-val")

			defaultMetricsCount++
			allMetricsCount++
			mb.RecordBpfTCPTxDataPoint(ts, 1, 3, "comm-val", "saddr-val", "daddr-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "bpf.tcp.rx":
					...
					attrVal, ok = dp.Attributes().Get("comm")
					assert.True(t, ok)
					assert.EqualValues(t, "comm-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("saddr")
					assert.True(t, ok)
					assert.EqualValues(t, "saddr-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("daddr")
					assert.True(t, ok)
					assert.EqualValues(t, "daddr-val", attrVal.Str())
				case "bpf.tcp.tx":
